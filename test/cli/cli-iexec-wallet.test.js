...
describe('iexec wallet', () => {
  ...
  describe('sweep', () => {
    test('iexec wallet sweep', async () => {
      const walletToSweep = await setRandomWallet(testChain)();
      ...
      const to = getRandomAddress();
      const res = await runIExecCliRaw(
        `${iexecPath} wallet sweep --to ${to} --force`,
      );
      ...
      const { balance: balanceTo } = await runIExecCliRaw(
        `${iexecPath} wallet show ${to}`,
      );
      const { balance: balanceFrom } = await runIExecCliRaw(
        `${iexecPath} wallet show ${walletToSweep.address}`,
      );

      expect(balanceTo.RLC).toBe(`${rlcBalance}.0`);
      expect(balanceTo.ether).not.toBe('0.0');

      expect(balanceFrom.RLC).toBe('0.0');
      expect(balanceFrom.ether).toBe('0.0');
    });
  });
});
