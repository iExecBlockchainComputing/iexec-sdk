...
class IExec {
  constructor(
    ...
    const getBridgedConf = async () => {
      if (!_bridgedConf) {
        ...
        if (isBridged) {
          ...
          const bridgedRpcUrl = bridgedNetworkConf.rpcURL !== undefined
          ...
          const bridgedBridgeAddress = bridgedNetworkConf.bridgeAddress !== undefined
    ...
    this.wallet.bridgeToSidechain = async (nRlcAmount) => wallet.bridgeToSidechain(
      await getContracts(),
    ...
    this.wallet.bridgeToMainchain = async (nRlcAmount) => wallet.bridgeToMainchain(
      await getContracts(),
      await getBridgeAddress(),
      nRlcAmount,
      {
    ...
    this.wallet.wrapEnterpriseRLC = async (nRlcAmount) => wallet.wrapEnterpriseRLC(
    ...
    this.dataset.checkDatasetSecretExists = async (datasetAddress) => secretMgtServ.checkWeb3SecretExists(
      await getContracts(),
      await getSmsURL(),
      datasetAddress,
    );
    this.dataset.pushDatasetSecret = async (datasetAddress, datasetSecret) => secretMgtServ.pushWeb3Secret(
      await getContracts(),
      await getSmsURL(),
      datasetAddress,
      datasetSecret,
    ...
    this.deal.fetchDealsByApporder = async (apporderHash) => order.fetchDealsByOrderHash(
      await getIexecGatewayURL(),
      order.APP_ORDER,
      await getChainId(),
      apporderHash,
    );
    this.deal.fetchDealsByDatasetorder = async (datasetorderHash) => order.fetchDealsByOrderHash(
      await getIexecGatewayURL(),
      order.DATASET_ORDER,
      await getChainId(),
      datasetorderHash,
    );
    this.deal.fetchDealsByWorkerpoolorder = async (workerpoolorderHash) => order.fetchDealsByOrderHash(
      await getIexecGatewayURL(),
      order.WORKERPOOL_ORDER,
      await getChainId(),
      workerpoolorderHash,
    );
    this.deal.fetchDealsByRequestorder = async (requestorderHash) => order.fetchDealsByOrderHash(
      await getIexecGatewayURL(),
      order.REQUEST_ORDER,
      await getChainId(),
      requestorderHash,
    ...
    this.order.publishApporder = async (signedApporder) => order.publishApporder(
      await getContracts(),
      await getIexecGatewayURL(),
      signedApporder,
    );
    this.order.publishDatasetorder = async (signedDatasetorder) => order.publishDatasetorder(
      await getContracts(),
      await getIexecGatewayURL(),
      signedDatasetorder,
    );
    this.order.publishWorkerpoolorder = async (signedWorkerpoolorder) => order.publishWorkerpoolorder(
      await getContracts(),
      await getIexecGatewayURL(),
    ...
    this.order.unpublishApporder = async (apporderHash) => order.unpublishApporder(
      await getContracts(),
      await getIexecGatewayURL(),
      apporderHash,
    );
    this.order.unpublishDatasetorder = async (datasetorderHash) => order.unpublishDatasetorder(
      await getContracts(),
      await getIexecGatewayURL(),
      datasetorderHash,
    );
    this.order.unpublishWorkerpoolorder = async (workerpoolorderHash) => order.unpublishWorkerpoolorder(
      await getContracts(),
      await getIexecGatewayURL(),
      workerpoolorderHash,
    );
    this.order.unpublishRequestorder = async (requestorderHash) => order.unpublishRequestorder(
      await getContracts(),
      await getIexecGatewayURL(),
      requestorderHash,
    );
    this.order.unpublishLastApporder = async (appAddress) => order.unpublishLastApporder(
      await getContracts(),
      await getIexecGatewayURL(),
      appAddress,
    );
    this.order.unpublishLastDatasetorder = async (datasetAddress) => order.unpublishLastDatasetorder(
      await getContracts(),
      await getIexecGatewayURL(),
      datasetAddress,
    );
    this.order.unpublishLastWorkerpoolorder = async (workerpoolAddress) => order.unpublishLastWorkerpoolorder(
      await getContracts(),
      await getIexecGatewayURL(),
      workerpoolAddress,
    );
    this.order.unpublishLastRequestorder = async () => order.unpublishLastRequestorder(
      await getContracts(),
      await getIexecGatewayURL(),
    );
    this.order.unpublishAllApporders = async (appAddress) => order.unpublishAllApporders(
      await getContracts(),
      await getIexecGatewayURL(),
      appAddress,
    );
    this.order.unpublishAllDatasetorders = async (datasetAddress) => order.unpublishAllDatasetorders(
      await getContracts(),
      await getIexecGatewayURL(),
      datasetAddress,
    );
    this.order.unpublishAllWorkerpoolorders = async (workerpoolAddress) => order.unpublishAllWorkerpoolorders(
      await getContracts(),
      await getIexecGatewayURL(),
      workerpoolAddress,
    );
    this.order.unpublishAllRequestorders = async () => order.unpublishAllRequestorders(
      await getContracts(),
      await getIexecGatewayURL(),
    );
    ...
    this.orderbook = {};
    this.orderbook.fetchApporder = async (apporderHash) => order.fetchPublishedOrderByHash(
      await getIexecGatewayURL(),
      order.APP_ORDER,
      await getChainId(),
      apporderHash,
    );
    this.orderbook.fetchDatasetorder = async (datasetorderHash) => order.fetchPublishedOrderByHash(
      await getIexecGatewayURL(),
      order.DATASET_ORDER,
      await getChainId(),
      datasetorderHash,
    );
    this.orderbook.fetchWorkerpoolorder = async (workerpoolorderHash) => order.fetchPublishedOrderByHash(
      await getIexecGatewayURL(),
      order.WORKERPOOL_ORDER,
      await getChainId(),
      workerpoolorderHash,
    );
    this.orderbook.fetchRequestorder = async (requestorderHash) => order.fetchPublishedOrderByHash(
      await getIexecGatewayURL(),
      order.REQUEST_ORDER,
      await getChainId(),
      requestorderHash,
    );
    this.orderbook.fetchAppOrderbook = async (appAddress, options = {}) => orderbook.fetchAppOrderbook(
      await getContracts(),
      await getIexecGatewayURL(),
      appAddress,
      options,
    ...
    this.storage.checkStorageTokenExists = async (address, { provider } = {}) => secretMgtServ.checkWeb2SecretExists(
      await getContracts(),
      await getSmsURL(),
      address,
